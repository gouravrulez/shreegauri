...
replace(`      if (d.data)\n        setS({ ...d.data, hero_image_url: "/maa-lakshmi-hero-fast.webp" } as S);`,`      if (d.data) {\n        setS({ ...d.data, hero_image_url: "/maa-lakshmi-hero-fast.webp" } as S);\n        if ((d.data as any).currency_rates) setCurrencyRates({ ...${JSON.stringify(DEFAULT_RATES)}, ...(d.data as any).currency_rates });\n      }`,`load admin currency rates`);
replace(`  }, []);\n  const money =`,`  }, []);\n  useEffect(() => {\n    const savedCurrency = localStorage.getItem("sg_currency");\n    if (savedCurrency && ${JSON.stringify(CURRENCIES)}.includes(savedCurrency)) setDisplayCurrency(savedCurrency);\n  }, []);\n  useEffect(() => { localStorage.setItem("sg_currency", displayCurrency); }, [displayCurrency]);\n  const money =`,`currency persistence`);
replace(`      <header>`,`      <div className="sg-currency-bar">\n        <label>Currency <select aria-label="Select display currency" value={displayCurrency} onChange={(e) => setDisplayCurrency(e.target.value)}>{${JSON.stringify(CURRENCIES)}.map((code) => <option key={code} value={code}>{code}</option>)}</select></label>\n        {displayCurrency !== "INR" && <small>Approximate converted price · Checkout/order base remains INR</small>}\n      </div>\n      <header>`,`currency selector UI`);
...
console.log("Shree Gauri international checkout and store-wide admin-backed currency display applied.");
