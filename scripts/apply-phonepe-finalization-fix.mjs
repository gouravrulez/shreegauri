...
const status = "app/api/payments/status/route.ts";
const hook = "app/api/payments/phonepe-webhook/route.ts";
...
s = s.replace(
  ...
  'p_payment_reference: String(phonePeStatus?.paymentDetails?.find((x) => String(x?.state || "").toUpperCase() === "COMPLETED")?.transactionId || phonePeStatus?.orderId || order.id),'
);
...
h = h.replace(
  ...
  'p_payment_reference: String(payload?.paymentDetails?.find((x) => String(x?.state || "").toUpperCase() === "COMPLETED")?.transactionId || payload?.orderId || merchantOrderId),'
);
