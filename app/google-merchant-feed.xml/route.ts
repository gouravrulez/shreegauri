...
export async function GET() {
  ...
  const items = (data || []).filter((p:any)=>absoluteImage(p.primary_image_url) && Number(p.price_inr)>0).map((p:any)=>{
    ...
    return `<item><g:id>${esc(p.id)}</g:id><title>${esc(p.name)}</title><description>${esc(p.short_description || p.description || p.name)}</description><link>${origin}/?product=${encodeURIComponent(p.slug || p.id)}</link><g:image_link>${esc(primary)}</g:image_link>${extras.map((x:string)=>`<g:additional_image_link>${esc(x)}</g:additional_image_link>`).join("")}<g:availability>${Number(p.stock_quantity)>0?"in_stock":"out_of_stock"}</g:availability><g:price>${Number(p.price_inr).toFixed(2)} INR</g:price><g:condition>new</g:condition><g:brand>${esc(p.brand || "Shree Gauri")}</g:brand><g:identifier_exists>false</g:identifier_exists>${p.google_product_category?`<g:google_product_category>${esc(p.google_product_category)}</g:google_product_category>`:""}</item>`;
  }).join("");
